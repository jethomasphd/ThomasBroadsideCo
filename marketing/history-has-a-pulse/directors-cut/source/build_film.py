"""Render the 60-second director's cut from the included sources. No API calls.
...
assert sum(s['duration'] for s in D['scenes'])==D['duration']==60
...
def mix_audio():
    ...
    fc += ['[0:a]asplit=2[ma][mb]', '[ma]atrim=0:52,asetpts=PTS-STARTPTS[ma1]',f'[mb]atrim=start={score_duration-9}:end={score_duration},asetpts=PTS-STARTPTS[mb1]', '[ma1][mb1]acrossfade=d=1:c1=tri:c2=tri,loudnorm=I=-24:TP=-4:LRA=8,afade=t=in:st=1.2:d=2,afade=t=out:st=58.5:d=1.5[music]']
    for i,v in enumerate(D['narration'],1):
        src=K/v['file'];actual=duration(src)/v['tempo'];assert v['at']+actual<=60
        ...
        voices.append(f'[v{i}]')
    fc.append(''.join(voices)+f'amix=inputs={len(voices)}:normalize=0,apad=whole_dur=60,asplit=3[voice][key1][key2]')
    for j,s in enumerate([s for s in D['scenes'] if s['kind']=='shop'],len(D['narration'])+1):
        ...
        shop.append(f'[p{j}]')
    fc.append(''.join(shop)+f'amix=inputs={len(shop)}:normalize=0,apad=whole_dur=60[shop]')
    fc += ['[music][key1]sidechaincompress=threshold=0.028:ratio=5:attack=30:release=450[mduck]', '[shop][key2]sidechaincompress=threshold=0.032:ratio=4:attack=20:release=300[pduck]', '[voice][mduck][pduck]amix=inputs=3:normalize=0,alimiter=limit=0.89:level=false,atrim=0:60,aformat=sample_rates=48000:channel_layouts=stereo[out]']
    run(inputs+['-filter_complex',';'.join(fc),'-map','[out]','-c:a','aac','-b:a','256k','-t','60',W/'soundtrack.m4a'])
...
def render(fmt):
    ...
    for i,s in enumerate(D['scenes']):
        ...
        run(input_args+['-t',s['duration'],'-an','-vf',','.join(vf),'-c:v','libx264','-preset','fast','-crf','19','-pix_fmt','yuv420p','-map_metadata','-1','-movflags','+faststart',dest])
    ...
    film=K/'films'/f'history-has-a-pulse-directors-cut-60s-{fmt}.mp4'
    run(['-i',silent,'-i',W/'soundtrack.m4a','-map','0:v:0','-map','1:a:0','-c','copy','-t','60','-map_metadata','-1','-movflags','+faststart',film])
    assert abs(duration(film)-60)<.05
    run(['-ss','3','-i',film,'-frames:v','1','-q:v','2',K/'artwork'/f'poster-{fmt}.jpg'])
    (K/'source'/f'timeline-{fmt}.json').write_text(json.dumps({'format':fmt,'duration':60,'shop_seconds':25.5,'captions_default':'off','shots':audit,'narration':D['narration']},indent=2)+'\n',encoding='utf-8')
...
cues=[(0,1.6,'[Press machinery]')]+[(v['at'],v['at']+v['duration'],v['text']) for v in D['narration']]
...
(K/'films/directors-cut-60s.vtt').write_text(vtt,encoding='utf-8')
(K/'films/directors-cut-60s.srt').write_text(srt,encoding='utf-8')
if args.web_root:
    m=args.web_root.resolve();m.mkdir(exist_ok=True,parents=True)
    run(['-i',K/'films/history-has-a-pulse-directors-cut-60s-wide.mp4','-vf','scale=1280:720','-c:v','libx264','-preset','fast','-crf','23','-c:a','aac','-b:a','160k','-map_metadata','-1','-movflags','+faststart',m/D['media']['film']])
    shutil.copy2(K/'films/directors-cut-60s.vtt',m/D['media']['captions'])
