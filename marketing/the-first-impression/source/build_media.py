...
clips={s:K/'source/clips'/f'{s}.mp4' for s in ['dunlap','franklin','jefferson','washington','hamilton']}
clips['opening']=clips['dunlap'];clips['austin']=K/'source/clips/austin.mp4'
if not clips['austin'].exists():shutil.copy2(R/'site/media/press/press-loop-wide.mp4',clips['austin'])
...
def film(name,fmt,segments,narrate=True):
    ...
    for n,(scene,duration) in enumerate(segments):
        ...
        run((['-loop','1'] if is_end else ['-stream_loop','-1'])+['-i',src,'-t',duration,'-an','-vf',','.join(filters),'-c:v','libx264','-preset','fast','-crf','21','-pix_fmt','yuv420p','-movflags','+faststart',dest])
...
run(['-i',K/'films/the-first-impression-65s.mp4','-vf','scale=1280:720','-c:v','libx264','-preset','fast','-crf','23','-c:a','aac','-b:a','128k','-movflags','+faststart',M/'the-first-impression-film.mp4'])
shutil.copy2(K/'films/the-first-impression-65s.vtt',M/'origin-film.vtt')
...
for suffix,vf in [('wide','scale=1280:720'),('portrait','scale=1280:720,crop=404:720:670:0,scale=540:960')]:
    run(['-i',clips['dunlap'],'-filter_complex',f'[0:v]trim=start=1:end=5,setpts=PTS-STARTPTS,{vf},fps=24,split[a][b];[b]reverse[r];[a][r]concat=n=2:v=1:a=0[v]','-map','[v]','-an','-c:v','libx264','-preset','fast','-crf','25','-pix_fmt','yuv420p','-movflags','+faststart',M/f'origin-loop-{suffix}.mp4'])
Image.open(K/'social/09-linkedin-share.png').convert('RGB').save(M/'share-card.jpg',quality=91,optimize=True)
