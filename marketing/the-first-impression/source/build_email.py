...
for slug,subject,kicker,heading,paragraphs,cta,target in letters:
    ...
    html=f'''<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{escape(subject)}</title><body style="margin:0;background:#e9e2d0"><div style="display:none;max-height:0;overflow:hidden">The First Impression. A printer’s night. A document on its way to the world.</div><table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:24px 12px"><table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;background:#f1ece0"><tr><td style="padding:32px;font:13px/1.5 'Courier New',monospace;letter-spacing:2px;border-bottom:1px solid #c3b8a3">THOMAS BROADSIDE CO.</td></tr><tr><td><a href="{escape(film)}"><img src="https://thomasbroadside.co/media/origin/share-card.jpg" width="600" alt="Before it was a relic, it was news. Watch The First Impression." style="display:block;width:100%;height:auto;border:0"></a></td></tr><tr><td style="padding:34px 32px"><p style="font:11px/1.6 'Courier New',monospace;letter-spacing:1px;color:#9e3123;text-transform:uppercase">{escape(kicker)}</p><h1 style="font:normal 38px/1.13 Georgia,serif;color:#201a10;margin:20px 0 28px">{escape(heading)}</h1>{body}<table role="presentation" cellpadding="0" cellspacing="0"><tr><td bgcolor="#9e3123" style="padding:16px 20px"><a href="{escape(url)}" style="font:13px/1.4 'Courier New',monospace;text-decoration:none;color:#faf6ea">{escape(cta)} →</a></td></tr></table><p style="font:14px/1.7 Georgia,serif;margin:25px 0"><a href="{escape(film)}#sources" style="color:#9e3123">Read the source notes and the film transcript</a></p><p style="font:italic 16px/1.6 Georgia,serif">The Shop Desk<br>Thomas Broadside Co.</p></td></tr><tr><td style="padding:25px 32px;border-top:1px solid #c3b8a3;font:12px/1.7 Georgia,serif;color:#675b47"><strong>BUILD SOMETHING THAT LASTS</strong><br>Thomas Broadside Co. · A venture of Thomas Graphics Inc.<br>9501 N Interstate Hwy 35, Austin, TX 78753<br><a href="mailto:{EMAIL}" style="color:#675b47">{EMAIL}</a><br><br>Historical scenes are contemporary reconstructions. The shop footage records our working Austin press.<br><a href="{unsub}" style="color:#675b47">Unsubscribe by email</a></td></tr></table></td></tr></table></body></html>'''
    plain=subject+'\n'+kicker+'\n\n'+'\n\n'.join(paragraphs)+f'\n\n{cta}: {url}\n\nSources and film: {film}\n\nThe Shop Desk\nThomas Broadside Co.\n9501 N Interstate Hwy 35, Austin, TX 78753\n{EMAIL}\n\nHistorical scenes are contemporary reconstructions.\nTo unsubscribe, reply with Unsubscribe.\n'
    (K/'email'/f'{slug}.html').write_text(html,encoding='utf-8');(K/'email'/f'{slug}-preview.html').write_text(html.replace('https://thomasbroadside.co/media/origin/share-card.jpg','../social/09-linkedin-share.png'),encoding='utf-8');(K/'email'/f'{slug}.txt').write_text(plain,encoding='utf-8')
    ...
    msg.add_alternative(html.replace('https://thomasbroadside.co/media/origin/share-card.jpg','cid:tfi-hero'),subtype='html')
    msg.get_payload()[-1].add_related((K.parents[1]/'site/media/origin/share-card.jpg').read_bytes(),maintype='image',subtype='jpeg',cid='<tfi-hero>')
