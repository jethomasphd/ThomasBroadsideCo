...
voice_duration={'room':7.059,'sea':8.731,'hill':8.220,'rest':7.198,'wood':4.644,'art':7.106,'closing':8.685}
titles={'room':'Words to live with.','sea':'A long way home.','hill':'The Beatitudes','rest':'A prayer on the page.','wood':'At the edge of the wood.','art':'Look a little longer.'}
labels={'room':'THE WESTERN CANON','sea':'HOMER / THE ODYSSEY / BOOK I','hill':'MATTHEW 5:7 / AUTHORIZED VERSION','rest':'AUGUSTINE / CONFESSIONS / BOOK I','wood':'DANTE / INFERNO / CANTO I','art':'THE COLLECTION / REMBRANDT & BLAKE'}
def clock(t):
...
def film(name,fmt,segments,narrate=True):
    ...
    for i,(scene,duration) in enumerate(segments):
        dest=W/f'{name}-{i}.mp4';end=scene=='closing';still=end or scene in ('art','rest')
        src=K/'source'/f'endcard-{fmt}.png' if end else K/'source/products'/('aristotle-with-a-bust-of-homer.jpg' if scene=='art' else 'our-heart-is-restless.jpg') if still else K/'source/clips'/f'{scene}.mp4'
        if still:
        ...
        vf += [f'fade=t=in:st=0:d=0.35',f'fade=t=out:st={duration-.35}:d=0.35']
        run((['-loop','1'] if still else ['-stream_loop','-1'])+['-i',src,'-t',duration,'-an','-vf',','.join(vf),'-c:v','libx264','-preset','fast','-crf','21','-pix_fmt','yuv420p','-movflags','+faststart',dest]);parts.append(dest);timeline.append((scene,position,duration));position+=duration;print(name,scene,flush=True)
...
film('words-to-live-with-70s','wide',[('room',9),('sea',11),('hill',12),('rest',10),('wood',8),('art',10),('closing',10)])
...
if M.exists():
    run(['-i',K/'films/words-to-live-with-70s.mp4','-vf','scale=1280:720','-c:v','libx264','-preset','fast','-crf','24','-c:a','aac','-b:a','128k','-movflags','+faststart',M/'words-to-live-with-film.mp4'])
    shutil.copy2(K/'films/words-to-live-with-70s.vtt',M/'canon-film.vtt')
